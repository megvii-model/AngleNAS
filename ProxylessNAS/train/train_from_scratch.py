...
parser.add_argument('--model_id', type=str, default='2 2 -1 -1 0 2 2 -1 5 -1 4 4 0 0 4 2 3 4 4 4 3', help='model_id')
...
def main():
  ...
  model = Network(rngs)
  flops, _ = profile(model, config.model_input_size_imagenet, rngs=rngs)
  params = count_parameters_in_MB(model)
  model = model.cuda(args.gpu)
  model = apex.parallel.DistributedDataParallel(model, delay_allreduce=True)

  logging.info('rngs:{}'.format(rngs))
